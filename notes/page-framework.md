...
实现采用生命周期事件连接各模块，将请求、内容、表现和业务分开。框架允许接入多层不同 UI：既可切换页面，也可切换当前页面内部的模式面板。
...
每类切换都有三层接口：外部调用统一请求接口，管理器与动画层使用流程细分接口，每个页面通过生命周期接口响应切换。新页面只需接入约定接口即可选择动画和内容行为，无需改写总调度流程。
